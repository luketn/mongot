...
public class IndexAnalyzerBuilderTest {
  ...
  private static SearchIndexDefinitionBuilder baseIndexDefinitionBuilder() {
    ...
    DocumentFieldDefinition mappings =
        DocumentFieldDefinitionBuilder.builder()
            ...
            .field("tokenNoNormalizer", tokenNoNormalizer)
  ...
  public void testBuild() throws Exception {
    ...
    Analyzer fieldAnalyzer = registry.getAnalyzer("fieldAnalyzer");
    Map<String, Analyzer> expected =
        Map.of(
            "$type:string/fieldWithMultiAnalyzer", fieldAnalyzer,
            "$multi/fieldWithMultiAnalyzer.multi", registry.getAnalyzer("multiAnalyzer"),
            "$type:string/fieldWithAnalyzer", fieldAnalyzer,
            "$type:string/fieldWithSearchAnalyzer", fieldAnalyzer,
            "$type:string/fieldWithNoAnalyzer", registry.getAnalyzer(DEFAULT_FALLBACK_ANALYZER),
            "$embedded:11/embeddedDoc/$type:string/embeddedDoc.nested_field", fieldAnalyzer,
            "$type:autocomplete/autocompleteField",
                registry.getAutocompleteAnalyzer(AUTOCOMPLETE_FIELD),
            "$type:token/tokenNoNormalizer", fallbackNormalizer,
            "$type:token/tokenWithNormalizer",
                registry.getNormalizer(StockNormalizerName.LOWERCASE));

    Map<String, Analyzer> result =
